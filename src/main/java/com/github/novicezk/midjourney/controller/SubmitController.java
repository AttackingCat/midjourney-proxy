...
public class SubmitController {
    ...
    public SubmitResultVO change(@RequestBody SubmitChangeDTO changeDTO) {
        ...
        } else if (TaskAction.REROLL.equals(changeDTO.getAction())) {
    ...
    public SubmitResultVO blend(@RequestBody SubmitZoomOutDTO zoomOutDTO) {
        ...
        Task targetTask = this.taskStoreService.get(zoomOutDTO.getTaskId());
        Task task = newTask(zoomOutDTO);
        task.setAction(TaskAction.ZOOMOUT);
        int messageFlags = targetTask.getPropertyGeneric(Constants.TASK_PROPERTY_FLAGS);
        String messageId = targetTask.getPropertyGeneric(Constants.TASK_PROPERTY_MESSAGE_ID);
        String messageHash = targetTask.getPropertyGeneric(Constants.TASK_PROPERTY_MESSAGE_HASH);
        return this.taskService.submitZoomOut(task, messageId, messageHash, messageFlags, (int) (magnification * 50));
    }
